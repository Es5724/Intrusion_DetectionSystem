...
- **메모리**: 최소 512MB RAM (고성능 모드는 1GB 권장)
...
1. **청크 기반 처리**
   - 패킷을 200개 단위로 나누어 처리
   - 즉시 처리 후 메모리 해제로 메모리 사용량 ~200KB 유지
...
3. **명시적 메모리 관리**
   - 사용 완료된 DataFrame 명시적 삭제 (`del df_chunk`)
   - 주기적 가비지 컬렉션 호출 (`gc.collect()`)
   - 패킷 버퍼 크기 제한 (최대 1,000개)
