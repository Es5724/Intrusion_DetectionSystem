...
if command -v suricata &> /dev/null; then
    ...
    if [[ $install_suricata =~ ^[Yy]$ ]]; then
        ...
        echo -e "${YELLOW}⚠️  수리카타 없이 계속 진행합니다. 경량 모드만 사용 가능합니다.${NC}"
...
while true; do
    ...
    case $choice in
        1)
            ...
            cd IDS
            $PYTHON_CMD IDSAgent_RL.py
            break
            ;;
        2)
            echo -e "${GREEN}⚡ 경량 모드로 실행 중...${NC}"
            cd IDS
            $PYTHON_CMD IDSAgent_RL.py --mode lightweight --no-menu
            break
            ;;
        3)
            if [ "$SURICATA_INSTALLED" = "1" ]; then
                ...
                cd IDS
                $PYTHON_CMD IDSAgent_RL.py --mode performance --no-menu
                break
            else
        ...
        4)
            ...
            cd IDS
            $PYTHON_CMD IDSAgent_RL.py --debug
            break
            ;;
        5)
...
install_suricata_func() {
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        if [ -f /etc/debian_version ]; then
            ...
            echo -e "${YELLOW}⚠️  지원하지 않는 Linux 배포판입니다.${NC}"
